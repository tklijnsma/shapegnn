...
def circle(x=0, y=0, r=1, n_points=100, angle=0.0):
    ...
    """    
    theta = np.linspace(0., 2.*np.pi, n_points+1)[:-1] + angle
    x = x + np.cos(theta) * r
    y = y + np.sin(theta) * r
    return x, y
...
def event(shuffle=True):
    ...
    def gen_coords():
        ...
        if extremity > 1.: r = min(abs(1-x_c), abs(-1-x_c), abs(1-y_c), abs(-1-y_c))
        n_points = int(60 * normal(1,.1) * r + 10)
    ...
    for _ in range(int(uniform(1,3))):
        ...
        x_shape, y_shape = polygon(x_c, y_c, r, angle=angle, n_sides=3, n_points=n_points)
        n_points = x_shape.shape[0]
        x_shape += normal(0., shape_noise, size=n_points)
        y_shape += normal(0., shape_noise, size=n_points)
    ...
    for _ in range(int(uniform(0,3))):
        x_c, y_c, angle, r, n_points = gen_coords()
        x_shape, y_shape = polygon(x_c, y_c, r, angle=angle, n_sides=4, n_points=n_points)
        n_points = x_shape.shape[0]
        x_shape += normal(0., shape_noise, size=n_points)
        y_shape += normal(0., shape_noise, size=n_points)
        X.append(np.column_stack((x_shape, y_shape)))
...
def draw_circle():
    with utils.quick_ax() as ax:
        ax.scatter(*circle(x=2, y=5, r=3, n_points=50, angle=.1*np.pi))
        ax.scatter(*circle(x=1, y=1, r=2, n_points=40, angle=.3*np.pi))
        x, y = circle(x=6, y=6, r=2, n_points=40, angle=.3*np.pi)
        x += normal(0., .1, size=x.shape)
        y += normal(0., .1, size=x.shape)
        ax.scatter(x, y)
        ax.set_xlim(-2, 10)
        ax.set_ylim(-2, 10)

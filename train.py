...
class ShapeGNN(nn.Module):
    ...
    def __init__(self, input_dim: int, output_dim: int=3, hidden_dim: int=16, k: int=16, n_edgeconvs: int=4):
        super().__init__()
...
class LossResult:
    ...
    def __add__(self, o):
        ...
        for c in o.components.keys():
            if not c in lr.components: lr.components[c] = 0.
...
class ObjectCondensation:
    def __init__(self, x, data, qmin=1., sB=.1):
        ...
        self.y_real = data.y[self.is_real] - 1 # Make it 0-indexed; normally 0 is the noise cluster
    ...
    def calc_q_betaclip(self):
        ...
        """
        self.q = (self.beta.clip(0., 1-1e-4)/1.002).arctanh()**2 + self.qmin
        self.calc_q_dependent_quantities()
    ...
    def V_att(self):
        if self.huberize_norm_for_V_att:
            ...
            # more.
            d = huber(self.d[self.is_real]+1e-5, 4.)
        else:
    ...
    def L_beta_sig_short_range_potential(self):
        ...
        closeness = (
            1./(20.*self.d[self.is_real]**2+1.) * self.M
            ).sum(dim=0)
        assert torch.all(closeness >= 1.) and torch.all(closeness <= self.n_per_cond)
        ...
        closeness = (closeness - 1.) / self.n_per_cond
        assert torch.all(closeness >= 0.) and torch.all(closeness <= 1.)
    ...
    def L_beta_sig_logterm(self):
        ...
        return (-.2*torch.log(self.beta_cond+1e-9)).mean()
...
def train():
    ...
    scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=0.05, patience=10, verbose=True)


    n_train = 20000
    fixed_test_ds = ShapeDataset(int(.125*n_train))
    ...
    def test():
        ...
        with torch.no_grad():
            ...
            loss = LossResult()
            for data in tqdm.tqdm(DataLoader(fixed_test_ds, batch_size=1, shuffle=False)):
                x = model(data)
                loss += loss_fn(x, data)
            print(f'test loss:\n{loss/len(fixed_test_ds)}')

            valid_ds = ShapeDataset(int(.125*n_train))
        ...
        for data in DataLoader(ds, batch_size=1, shuffle=False):
            ...
            with utils.quick_fig((8*n_cols,8*n_rows)) as fig:
                ...
                for y in np.unique(data.y):
                    ...
                    if do_clustering: y_to_truth_cluster[y].color = pc.get_facecolor()
                ...
                ax.hist(oc.beta[~is_noise], bins=bins, label='shapes', density=True, alpha=.5)
                ax.legend()
                ax.set_xlabel('beta')
                ...
                if do_clustering:
                    ...
                    for c in clusters:
                        ax.scatter(data.x[c.in_cluster, 0], data.x[c.in_cluster, 1], c=c.color)
                    ax.set_xlim(-1, 1)
                    ax.set_ylim(-1, 1)
                    ax.set_title('Normal space (pred-colored)')

                    ax = fig.add_subplot(n_rows,n_cols,5)
                    ax.scatter(x[is_pred_noise, 0], x[is_pred_noise, 1], s=s[is_pred_noise])
                    for c in clusters:
                        ax.scatter(x[c.in_cluster, 0], x[c.in_cluster, 1], c=c.color, s=s[c.in_cluster])
...
def make_clusters(beta, x, cluster_core_radius=.1, min_core_charge=1.5, cluster_radius=.2):
